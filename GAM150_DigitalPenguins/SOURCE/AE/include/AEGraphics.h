...
typedef	enum
{
	AE_GFX_RM_NONE,		// No shader enabled.
	AE_GFX_RM_COLOR,	// Meshes rendered using colored vertices.
	AE_GFX_RM_TEXTURE,	// Meshes rendered using UV texture coordinates.

	AE_GFX_RM_NUM		// Total number of render modes supported.
}AEGfxRenderMode;
...
typedef enum
{
	AE_GFX_BM_NONE = 0,	// Computed color replaces existing color.
	AE_GFX_BM_BLEND,	// Computed color blended with existing color (using alpha).
	AE_GFX_BM_ADD,		// Computed color added with existing color.
	AE_GFX_BM_MULTIPLY,	// Computed color multiplied with existing color.

	AE_GFX_BM_NUM		// Total number of blend modes supported.
}AEGfxBlendMode;
...
typedef enum
{
	AE_GFX_TM_PRECISE = 0,	// Texture sampling returns nearest pixel.
	AE_GFX_TM_AVERAGE		// Texture sampling returns average of nearest pixels.
...
typedef enum
{
	AE_GFX_MDM_POINTS = 0,	// Meshes rendered as a series of points.
	AE_GFX_MDM_LINES,		// Meshes drawn as a series of non-contiguous line segments (two vertices per line).
	AE_GFX_MDM_LINES_STRIP,	// Meshes drawn as a series of contiguous line (starting vertex plus one vertex per line).
	AE_GFX_MDM_TRIANGLES,	// Meshes drawn as a series of triangles (three vertices per triangle)
	
	AE_GFX_MDM_NUM		// Total number of draw modes supported.
}AEGfxMeshDrawMode;
...
typedef enum
{
	AE_WOM_NORMAL = 0,	// World origin is independent of the screen. Objects are drawn relative to the camera.
	AE_WOM_TOP_LEFT,	// World origin is located at the top-left corner of the screen.
	
	AE_WOM_NUM,			// Total number of world origin modes supported.
}WorldOriginMode;
 ...
 * @details Adds a triangle (3 vertices) to the mesh being created.
 * 
 * @param x0 x coordinate of the first vertex
 * @param y0 Y coordinate of the first vertex
 * @param c0 ARGB values for the first vertex (ie 0xFF00FFFF)
 * @param tu0 Horizontal texture coordinate
 * @param tv0 Vertical texture coordinate
 * @param x1 X coordinate of the second vertex
 * @param y1 Y coordinate of the second vertex
 * @param c1 ARGB values for the second vertex
 * @param tu1 Horizontal texture coordinate
 * @param tv1 Vertical texture coordinate
 * @param x2 X coordinate of the third vertex
 * @param y2 Y coordinate of the third vertex
 * @param c2 ARGB values for the third vertex
 * @param tu2 Horizontal texture coordinate
 * @param tv2 Vertical texture coordinate
 */
 ...
 * @param c0 ARGB values for the vertex (ie 0xFF00FFFF)
 * @param tu0 Horizontal texture coordinate
 * @param tv0 Vertical texture description
 */
